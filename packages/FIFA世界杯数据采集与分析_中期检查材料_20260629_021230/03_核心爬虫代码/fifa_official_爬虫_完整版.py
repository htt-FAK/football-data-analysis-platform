...
class FIFAOfficialCrawler(BaseCrawler):
    ...
    def _crawl_statistics(self, **kwargs):
        ...
        team_stats = self._build_team_statistics(standings, finished_group_matches)
    ...
    def _build_team_statistics(
        ...
        for match in matches:
            ...
            per_team = self._aggregate_match_team_statistics(match, team_ids_by_name)
            if not per_team:
                continue
    ...
    def _aggregate_match_team_statistics(
        ...
        for player_stats in stats_payload.values():
            metric_map = self._stat_rows_to_map(player_stats)
            team_name = self._resolve_match_stat_team_name(metric_map, home_name, away_name, home_team_key, away_team_key)
            if not team_name:
                continue
