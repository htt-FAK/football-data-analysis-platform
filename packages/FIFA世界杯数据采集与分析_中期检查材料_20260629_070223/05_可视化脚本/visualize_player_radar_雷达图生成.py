...
# Player name mapping (English to Chinese)
PLAYER_NAME_CN = {
    'Lionel MESSI': '梅西',
    'Kylian MBAPPE': '姆巴佩',
...
def generate_player_radar_charts(csv_path, output_dir, top_n=8):
    ...
    for idx, (_, row) in enumerate(df_sorted.iterrows()):
        ...
        # Player info - use Chinese names when available
        player_name_en = row['球员姓名']
        player_name_cn = PLAYER_NAME_CN.get(player_name_en, player_name_en)
        team_en = row['球队']
        team_cn = TEAM_NAME_CN.get(team_en, team_en)
        position = row['位置']
        group = row['分组']
        
        title = f"{player_name_cn}\n{team_cn} | {position} | {group}"
        
        # Draw radar
        draw_hexagon_radar(ax, normalized_stats, labels, title)
        
        # Save figure with Chinese name in filename
        safe_name = player_name_cn.replace('·', '_').replace(' ', '_')
        filename = f"player_{idx+1}_{safe_name}.png"
        filepath = output_path / filename
        plt.savefig(filepath, dpi=150, bbox_inches='tight', facecolor='white')
        plt.close()
        
        print(f"Generated: {filename} ({player_name_en} -> {player_name_cn})")
...
def generate_comparison_chart(df_sorted, output_path):
    ...
    ax.set_xticks(x)
    # Use Chinese names for comparison chart
    player_labels = []
    for _, row in df_sorted.iterrows():
        name_en = row['球员姓名']
        name_cn = PLAYER_NAME_CN.get(name_en, name_en)[:8]
        player_labels.append(name_cn)
    
    ax.set_xticklabels(player_labels, rotation=45, ha='right', fontsize=9)
...
if __name__ == '__main__':
    ...
    stats_files = sorted(
        export_dir.glob("worldcup_group_stage_player_stats_*.csv"),
        key=lambda path: path.stat().st_mtime,
        reverse=True,
    )
    if not stats_files:
        raise FileNotFoundError(f"未找到球员统计 CSV: {export_dir}")

    csv_file = stats_files[0]
    
    # Output directory
    output_dir = export_dir / "radar_charts"
